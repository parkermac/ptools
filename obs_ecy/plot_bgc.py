...
if save_fig==True:
    ...
    Lfun.make_dir(dir11)
    dir1 = dir11 + 'bgc_series' + str(year) + '_' + Ldir['gtagex'] + '/'
        ...
        try:
            ...
            for Z in bo.index:
                ...
                for vn in mod_vn_dict.keys():
                    ...
                    # debugging
                    if (vn == 'oxygen') and False:
                        v = ((1-fr)*mcv[int(i0)] + fr*mcv[int(i1)])
                        mfd = mod_fac_dict[vn]
                        print(fnm)
                        print('i0=%d, i1=%d, fr=%0.2f' % (i0, i1, fr))
                        print('v=%0.1f, mfd=%0.4f, val=%0.1f' % (v, mfd, val))
            ds.close()
        except OSError:
            pass
...
for station in sta_to_plot:
    ...
    for vn in ['Salinity', 'Temp. (deg C)', 'DO (mg L-1)', 'DIN (uM)']:
        ...
        if pp==1:
            ...
            ax.text(.05, .1, 'Solid = Observed, Dashed = Model', color='k', transform=ax.transAxes)
        if pp in [1,2]:
            ax.set_xticklabels('')
            ax.set_xlabel('')
        else:
