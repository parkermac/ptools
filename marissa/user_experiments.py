...
def get_ic(EI, fn00):
    ...
    if exp_name == 'p5_PS':
        ...
        import pfun
        ds = nc.Dataset('../../../parker/LiveOcean_data/grids/cas6/grid.nc')
        latbounds = [47.581, 47.663]
        lonbounds = [-122.441, -122.331]

        lats = ds['lat_psi'][:].data
        lons = ds['lon_psi'][:].data
        maskr = ds['mask_rho'][:].data

        # latitude lower and upper index

        rlat = lats[:,1]>=latbounds[0]
        latin = np.argmin(~rlat)

        rlat = lats[:,1]<=latbounds[1]
        latax = np.argmax(~rlat)

        rlon = lons[1,:]>=lonbounds[0]
        lonin = np.argmin(~rlon)

        rlon = lons[1,:]<=lonbounds[1]
        lonax = np.argmax(~rlon)

        maskr = maskr[latin:latax, lonin:lonax]
        print(maskr)
        lat = lats[latin:latax, 1]
        lon = lons[1, lonin:lonax]
